...
function BrickInterfaceDefinition(id, constructor) {
	var self = this;
	self.__ = { definitionType : "BrickInterfaces" };
...
function BrickInterface(definition, constructor) {
	...
	function createInterfacedBrick(options, callback) {	
		var concreteBrick = options.brick.___ || options.brick;
		var interfacedBrick = {};
		forEachMember({ type : "methods" }, function(member, nextMember) {
			var memberName = member.name;
			var method = getMemberValue(concreteBrick, memberName);
			...
			if (err) { callback(err); } else {
				forEachMember({ type : "properties" }, function(member, nextMember) {
					var memberName = member.name;
					var propertyValue = getMemberValue(concreteBrick, memberName);
					setMemberValue(interfacedBrick, memberName, propertyValue);
					nextMember();
				}, function(err) {
					if (err) { callback(err); } else {
						interfacedBrick._ = concreteBrick._;
						interfacedBrick.__ = concreteBrick.__;
						interfacedBrick.___ = concreteBrick;
