...
function BrickTypeDefinition(name, id, constructor) {
	var self = this;
	self.__ = { definitionType : "BrickTypes" };
...
function BrickType(env, definition, constructor) {
	...
	function createInstance(options, callback) {
		...
		brick.__ = { id : newBrickId, brickTypeId : definition.id };
		brick._ = { };
		callback(null, brick);
	}
	...
	function forEachMember(options, memberCallback, callback) {
		...
		async.forEachSeries(Object.keys(membersOfType), function(memberName, nextMember) {
			...
			var memberCallbackOptions = { name : memberName, member : member, brickType : self };
			memberCallback(memberCallbackOptions, nextMember)
	...
	function initialiseBrick(options, callback) {
		createInstance({}, function(err, brick) {
			if (err) {callback(err);} else {
				...
				var initialisedBricks = options.batch.initialisedBricks;
				...
				forEachMember({ type : "properties" }, function(propItem, nextProperty) {
					...
					forEachPropertyValue({ property : propItem.member, value : propertyValue }, function(propValue, nextPropValue) {
						env.methods.BrickPropertyTypes.initialise({ property : propItem.member, value : propValue.value, batch : options.batch }, function(err, initialisedValue) {
					...
					if (err) { callback(err); } else { 
						brick._.initialised = true;
						brick._.brickType = self;
						callback(null, brick);
					}
				});
			}
	...
	function deinitialiseBrick(options, callback) {
		...
		var persistedBricks = options.batch.persistedBricks || {};
		persistedBricks[brick.__.id] = true;
		
		var brickJson = { __ : ce.clone(brick).__ };
		delete brickJson._;
		delete brickJson.__.brickType; //keep for time being, need to clean up!
		
		forEachMember({ type : "properties" }, function(propItem, nextProperty) {
			var propertyValue = getPropertyValue(brick, propItem.name);
			forEachPropertyValue({ property : propItem.member, value : propertyValue }, function(propValue, nextPropValue) {
				env.methods.BrickPropertyTypes.deinitialise({ property : propItem.member, value : propValue.value, batch : options.batch }, function(err, deinitialisedValue) {
	...
	function buildSummary(obj, keyPrefix) {
		...
		Object.keys(obj).forEach(function(key) {
			...
			if (member instanceof BrickTypeState) { summary.state[fullKey] = member; }
			else if (member instanceof BrickTypeProperty) { summary.properties[fullKey] = member; }
			else if (member instanceof BrickTypeMethod) { summary.methods[fullKey] = member; }
			else if (typeof(member) == "object"){ ce.extend(summary, buildSummary(member, fullKey)); }
		});
		return summary;
	}
}
