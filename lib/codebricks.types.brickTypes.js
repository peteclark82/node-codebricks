...
function BrickType(env, definition, constructor) {
	...
	function forEachProperty(options, propertyCallback, callback) {
		async.forEachSeries(Object.keys(self.__.properties), function(key, nextProperty) {
			...
			propertyCallback({ name : key, property : property, value : propertyValue, brickType : self }, nextProperty)
		}, function(err) { if (err) { callback(err); } else { callback(); } });	
	}
	...
	function deserializeBrick(options, callback) {
		createInstance({}, function(err, brick) {
			if (err) {callback(err);} else {
				...
				forEachProperty({ brick : brick }, function(propItem, nextProperty) {
					forEachPropertyValue({ property : propItem.property, value : propItem.value }, function(propValue, nextPropValue) {
						env.methods.BrickPropertyTypes.deserialize({ property : propItem.property, value : propValue.value }, function(err, deserializedValue) {
							if (err) { nextPropValue({ message : "Error deserializing property '"+ propItem.name + "' on brick id '"+ brick.__.id +"'", error : err, _brickJson : options.brickJson }); } else {
								nextPropValue(null, deserializedValue);
							}
						});
					}, function(err, finalValue) {
						if (err) { nextProperty(err); } else {
							propItem.brickType.setPropertyValue(brick, propItem.name, finalValue);
							nextProperty();
						}
					});
				}, function(err) {
	...
	function deinitialiseBrick(options, callback) {
		...
		forEachProperty({ brick : brick }, function(propItem, nextProperty) {
			forEachPropertyValue({ property : propItem.property, value : propItem.value }, function(propValue, nextPropValue) {
				env.methods.BrickPropertyTypes.deinitialise({ property : propItem.property, value : propValue.value, depth : options.depth, persistedBricks : persistedBricks }, function(err, deinitializedValue) {
					if (err) { nextPropValue(err); } else {
						nextPropValue(null, deinitializedValue);
					}
				});
			}, function(err, finalValue) {
				if (err) { nextProperty(err); } else {
					propItem.brickType.setPropertyValue(brickJson, propItem.name, finalValue);
					nextProperty();
				}
			});
		}, function(err) {
