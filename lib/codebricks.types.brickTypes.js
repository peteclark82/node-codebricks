...
function BrickType(definition, constructor) {
	...
	function forEachMember(options, propertyCallback, callback) {
		var members = self.__[options.type];
		if (members === undefined) {
		} else {
			async.forEachSeries(Object.keys(members), function(memberName, nextMember) {
	...
	function forEachPropertyValue(options, valueCallback, callback) {
		...
		async.forEachSeries(initialValues, function(propertyValueItem, nextPropertyValue) {
			...
			if (err) {callback(err);} else {
				var finalValue = propertyType.isEmpty({ value : finalValues }) ? options.value : finalValues;
