			...
			if (err) {callback(err);} else {
				...
				forEachMember({ type : "properties" }, function(propItem, nextProperty) {
					...
					forEachPropertyValue({ property : propItem.member, value : propertyValue }, function(propValue, nextPropValue) {
						env.methods.BrickPropertyTypes.initialise({ property : propItem.member, value : propValue.value, context : options.context, depth : options.depth, initialisedBricks : initialisedBricks }, function(err, initialisedValue) {
