		...
		fs.exists(self.path, function(exists) {
			...
			fs.exists(filename, function(exists) {
				...
				fs.readFile(filename, function(err, brickJsonStream) {
					...
					env.methods.BrickTypes.initialiseBrick({ brickJson : brickJson, context : options.context, depth : options.depth, initialisedBricks : options.initialisedBricks }, callback);
				});
			});
		});
	}
