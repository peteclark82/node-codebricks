...
module.exports = new cb.internal.types.brickTypes.BrickTypeDefinition("FileSystemRepository", "codebricks.brickRepositories.fileSystem", function(env, builder) {
	...
	function save(options, callback) {
		fs.exists(self.path, function(exists) {
			...
			env.methods.BrickTypes.deinitialiseBrick({ brick : options.brick, depth : options.depth, persistedBricks : options.persistedBricks }, function(err, brickJson) {
		...
		fs.exists(self.path, function(exists) {
			...
			fs.exists(filename, function(exists) {
				...
				fs.readFile(filename, function(err, brickJsonStream) {
					...
					env.methods.BrickTypes.initialiseBrick({ brickJson : brickJson, depth : options.depth, initialisedBricks : options.initialisedBricks, brickContext : options.brickContext }, callback);
				});
			});
		});
	}
