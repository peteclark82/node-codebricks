		...
		fs.exists(self.path, function(exists) {
			...
			fs.exists(filename, function(exists) {
				...
				fs.readFile(filename, function(err, brickJsonStream) {
					...
					env.methods.BrickTypes.deserializeBrick({ brickJson : brickJson, depth : options.depth, resolveCache : options.resolveCache, brickContext : options.brickContext }, callback);
				});
			});
		});
	}
