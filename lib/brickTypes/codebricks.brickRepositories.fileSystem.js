	...
	function save(options, callback) {
		fs.exists(self.path, function(exists) {
			if (!exists) {
				...
				env.methods.BrickTypes.serializeBricks({ brick : options.brick, deep : options.deep }, function(err, brickJsonArray) {
					async.forEachSeries(brickJsonArray, function(brickJson, nextBrickJson) {
						...
						console.log(require("util").inspect(brickJson, null, null, true));
