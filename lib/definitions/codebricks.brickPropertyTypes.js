		...
		getPropertyType({ type : options.property.type}, function(err, propertyType) {
			if (err) {callback(err);} else {
				if (propertyType.initialise === undefined) {
					...
					propertyType.initialise({ params : options.property.params, value : options.value, depth : options.depth, initialisedBricks : options.initialisedBricks }, function(err, value) {
						if (err) { callback(err); } else { 
							callback(null, value);
						}
					});
