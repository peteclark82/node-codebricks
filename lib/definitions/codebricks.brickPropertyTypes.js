		...
		getPropertyType({ type : options.property.type}, function(err, propertyType) {
			if (err) {callback(err);} else {	
				if (propertyType.resolveProperty === undefined) {
					...
					propertyType.resolveProperty({ brick : options.brick, value : options.value, depth : options.depth }, function(err, newValue) {
						if (err) { callback(err); } else {
							initialise({ property : options.property, value : newValue }, function(err, initialisedValue) {
								if (err) { nextPropValue({ message : "Error initialising property '"+ options.property.name + "' on brick id '"+ brick.__.id +"'", error : err }); } else {
