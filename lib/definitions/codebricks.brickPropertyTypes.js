...
module.exports = new cb.internal.types.DefinitionType("BrickPropertyTypes", cb.internal.types.brickPropertyTypes.BrickPropertyTypeDefinition, function(env) {
	...
	self.unresolveProperty = unresolveProperty;
		...
		getPropertyType({ type : options.property.type}, function(err, propertyType) {
			if (err) {callback(err);} else {
				if (propertyType.deinitialise === undefined) {
					...
					propertyType.deinitialise({ params : options.property.params, value : options.value, depth : options.depth, persistedBricks : options.persistedBricks }, function(err, value) {
						if (err) { callback(err); } else { callback(null, value); }
					});
				}
			}
	...
	function unresolveProperty(options, callback) {
		getPropertyType({ type : options.property.type}, function(err, propertyType) {
			if (err) {callback(err);} else {	
				if (propertyType.unresolveProperty === undefined) {
					callback(null, options.value);
				} else {	
					propertyType.unresolveProperty({ value : options.value, depth : options.depth, resolveCache : options.resolveCache }, function(err, newValue) {
						if (err) { callback(err); } else {
							deinitialise({ property : options.property, value : newValue }, function(err, initialisedValue) {
								if (err) { callback(err); } else {
									callback(null, initialisedValue);
								}
							});		
