...
module.exports = new cb.internal.types.DefinitionType("BrickTypeDefinition", cb.internal.types.brickTypes.BrickTypeDefinition, function(env) {
	...
	self.deserializeBricks = deserializeBricks;
	...
	function getBrick(options, callback) {
		if (repositories.length == 0) { callback({ message : "No repository brick registered." }) } else {
			...
			repositoryBrick.get({ id : options.id, deep : options.deep }, function(err, brick) {
				if (err) { callback({ message : "Error getting brick id '"+ options.id +"'", error : err }); } else {
					callback(null, brick);
				}
			});
		}
	...
	function deserializeBricks(options, getBrickCallback, callback) {
		getBrickCallback({ id : options.id }, function(err, brickJson) {
			if (err) { callback(err); } else {
				deserializeBrick(brickJson, function(err, brick) {
					if (err) {callback(err);} else {
						if (options.deep !== true) {
							...
							if (resolvedBricks[options.id] !== undefined) {
								...
								resolveAllReferencedBricks({ brick : brick }, function(brick, nextBrick) {
									deserializeBricks({ id : brick.__.id, deep : options.deep, resolvedBricks : resolvedBricks }, getBrickCallback, nextBrick);
								}, function(err) {
									if (err) {callback(err);} else {
										callback(null, brick);
									}
	...
	function assertValidBrick(options, callback) {
		...
		if (isValid === false) {
			callback({ message : "JSON is not a valid codebrick", jsonData : brick });
		} else {
			callback();
		}
	}
	...
	function resolveAllReferencedBricks(options, resolveCallback, callback) {	
		loopBrickProperties({ brick : options.brick }, function(prop, nextProperty) {
			loopBrickPropertyValues({ type : prop.type, value : prop.value }, function(propValue, nextPropValue) {
				env.methods.BrickPropertyTypeDefinition.getReferencedBrick({ type : propValue.type, value : propValue.value }, function(err, nestedBrick) {					
					if (err) { nextPropValue({ message : "Error getting bricks for property '"+ prop.name + "' on brick id '"+ options.brick.__.id +"'", error : err }); } else {
