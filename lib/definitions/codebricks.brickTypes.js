...
module.exports = new cb.internal.types.DefinitionType("BrickTypes", cb.internal.types.brickTypes.BrickTypeDefinition, function(env) {
		...
		env.getDefinition(cb.internal.types.brickTypes.BrickTypeDefinition, options.brick.__.brickTypeId, function(err, brickTypeDef) {
			if (err) {callback(err); } else {
				...
				async.forEachSeries(Object.keys(brickType.__.properties), function(key, nextProperty) {
					var propertyType = brickType.getPropertyType(key);
					var propertyValue = brickType.getPropertyValue(options.brick, key);										
					propertyCallback({ name : key, type : propertyType, value : propertyValue, brickType : brickType }, nextProperty)
	...
	function loopBrickPropertyValues(options, valueCallback, callback) {
		...
		switch(propertyType.mode) {
			case "array":
				finalValues = [];
				if (propertyValue) {
					propertyValue.forEach(function(value, index) {
						propertyValues.push({ key : index, value : value })
					});
				}
				break;
			case "hash":
				...
				if (propertyValue) {
					Object.keys(propertyValue).forEach(function(key) {
						propertyValues.push({ key : key, value : propertyValue[key] })
					});
				}
				break;
			case "single" : default:
				finalValues = undefined;
				propertyValues.push({ key : null, value : propertyValue })
				break;
		}
		
		async.forEachSeries(propertyValues, function(propertyValue, nextPropertyValue) {
			valueCallback({ type : propertyType, value : propertyValue.value }, function(err, value) {
				if (err) { nextPropertyValue(err); } else {
					if (value !== undefined) {
						switch(propertyType.mode) {
							case "array":
								finalValues.push(value); break;
							case "hash":
								finalValues[propertyValue.key] = value; break;
								break;
							case "single" : default:
								finalValues = value; break;
						}
					}
					nextPropertyValue();
				}
			...
			if (err) {callback(err);} else {
				...
				switch(propertyType.mode) {
					case "array":
						finalValue = finalValues.length > 0 ? finalValues : undefined; break;
					case "hash":
						finalValue = Object.keys(finalValues).length > 0 ? finalValues : undefined; break;
						break;
					case "single" : default:
						finalValue = finalValues; break;
				}
