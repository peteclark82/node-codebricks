		...
		assertValidBrick({ brick : brickJson }, function(err) {
			if (err) { callback(err); } else {
				getBrickType({ brickTypeId : brickJson.__.brickTypeId }, function(err, brickType) {
					if (err) { callback(err); } else {
						brickType.initialiseBrick({ brickJson : brickJson, context : options.context, depth : options.depth, initialisedBricks : options.initialisedBricks}, function(err, brick) {
							if (err) {callback(err);} else {	
