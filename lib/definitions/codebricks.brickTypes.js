					...
					if (err) {callback(err);} else {
						if (options.deep !== true) {
							...
							if (resolvedBricks[options.id] !== undefined) {
								...
								resolveAllReferencedBricks({ brick : brick }, function(unresolvedBrick, nextBrick) {
									deserializeBricks({ id : unresolvedBrick.__.id, deep : options.deep, resolvedBricks : resolvedBricks }, getBrickCallback, function(err, resolvedBrick) {
										if (err) {
											callback({ message : "Error resolving reference on brick id '"+ brick.__.id +"'", error : err });
										} else {
											callback(null, resolvedBrick);
										}
									});
								}, function(err) {
									if (err) {callback(err);} else {
